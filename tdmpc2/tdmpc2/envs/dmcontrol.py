...
class TimeStepToGymWrapper:
    ...
    def step(self, action):
        ...
        obs = self._obs_to_array(time_step.observation).astype(np.float32)
        info = defaultdict(float)
        try:
            info["x_velocity"] = self.env.physics.velocity()[0]
        except (AttributeError, IndexError):
            pass
        if "x_velocity" in info:
            info["move"] = info["x_velocity"]
        info["success"] = self.env.task.get_termination(self.env.physics)
        return (
            obs,
            time_step.reward,
            time_step.last(),
