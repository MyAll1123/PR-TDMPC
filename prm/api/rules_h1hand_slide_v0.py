...
"""Slide任务的自包含规则：基于humanoid_bench Slide任务的真实奖励函数设计

自包含设计，无外部依赖，基于humanoid_bench Slide任务的奖励函数实现轨迹比较。
"""

import numpy as np

def compute_slide_reward_components(trajectory):
    """
    基于humanoid_bench真实奖励函数计算Slide任务的奖励组件
    ...
    sliding_stability_scores = []
    forward_sliding_scores = []
    ...
    for i, obs in enumerate(observations):
        ...
        sliding_stability = standing_score * upright_score
        sliding_stability_scores.append(sliding_stability)
        
        # 2. 前向滑行效率 (35%)
        # 基于move组件
        move_score = max(0.0, min(1.0, velocity_x / 1.0)) if velocity_x > 0 else 0.0
        forward_sliding_scores.append(move_score)
    ...
    # 计算平均分数
    avg_sliding_stability = np.mean(sliding_stability_scores) if sliding_stability_scores else 0.0
    avg_forward_sliding = np.mean(forward_sliding_scores) if forward_sliding_scores else 0.0
    avg_control_efficiency = np.mean(control_efficiency_scores) if control_efficiency_scores else 0.0
    
    return avg_sliding_stability, avg_forward_sliding, avg_control_efficiency

def compute_trajectory_score(trajectory):
    """
    计算轨迹的综合得分
    
    基于humanoid_bench真实奖励函数的三个核心组件：
    1. 滑行高度稳定性 (40%): standing * upright
    2. 前向滑行效率 (35%): move
    3. 控制效率 (25%): small_control
    """
    sliding_stability, forward_sliding, control_efficiency = compute_slide_reward_components(trajectory)
    ...
    # 计算综合得分
    total_score = w_sliding * sliding_stability + w_motion * forward_sliding + w_control * control_efficiency
    
    return total_score

def evaluate_slide_trajectory(trajectory):
    """
    评估单个轨迹的质量
    ...
    """
    sliding_stability, forward_sliding, control_efficiency = compute_slide_reward_components(trajectory)
    total_score = compute_trajectory_score(trajectory)
    
    return {
        'sliding_stability': sliding_stability,
        'forward_sliding': forward_sliding,
...
def evaluate_dpo_preference(trajectory_1, trajectory_2, goal=None):
    """
    基于humanoid_bench真实奖励函数评估Slide任务的DPO偏好
    ...
    """
    # 计算两个轨迹的奖励组件
    sliding_1, motion_1, control_1 = compute_slide_reward_components(trajectory_1)
    sliding_2, motion_2, control_2 = compute_slide_reward_components(trajectory_2)
    ...
    # 计算综合奖励
    reward_1 = w_sliding * sliding_1 + w_motion * motion_1 + w_control * control_1
    reward_2 = w_sliding * sliding_2 + w_motion * motion_2 + w_control * control_2
...
def compare_h1hand_slide_v0_trajectories(tau_1, tau_2, goal):
    """h1hand-slide-v0 任务的专用比较规则"""
    return compare_slide_trajectories(tau_1, tau_2, goal)

def compare_slide_trajectories(tau_1, tau_2, goal):
    """
    基于humanoid_bench真实奖励函数的Slide场景启发式偏好对比规则
