...
"""Insert Normal任务的启发式规则：基于轨迹质量评估的DPO偏好标签生成
...
def compare_h1hand_insert_normal_v0_trajectories(tau_1, tau_2, goal):
    """h1hand-insert_normal-v0 任务的专用比较规则 - 基于humanoid_bench真实奖励函数设计"""
    ...
    for state in states:
        if isinstance(state, (list, np.ndarray)) and len(state) > 20:
            try:
                ...
                # 使用tolerance函数逻辑计算奖励
                target_a = _compute_tolerance_reward(distance_a, margin=0.5, sigmoid="linear")
                target_b = _compute_tolerance_reward(distance_b, margin=0.5, sigmoid="linear")
    ...
    for state in states:
        if isinstance(state, (list, np.ndarray)) and len(state) > 25:
            try:
                ...
                # 使用tolerance函数逻辑
                height_reward_a = _compute_tolerance_reward(height_diff_a, margin=0.15, sigmoid="linear")
                height_reward_b = _compute_tolerance_reward(height_diff_b, margin=0.15, sigmoid="linear")
    ...
    for state in states:
        if isinstance(state, (list, np.ndarray)) and len(state) > 15:
            try:
                ...
                # 使用tolerance函数逻辑
                proximity_score = _compute_proximity_reward(min_distance, bounds=(0, 0.2), margin=0.5)
...
def _compute_small_control_reward(actions) -> float:
    ...
    try:
        ...
        small_control = _compute_tolerance_reward(avg_force, margin=10, sigmoid="quadratic")
...
def _compute_proximity_reward(distance, bounds=(0, 0.2), margin=0.5) -> float:
    """
    计算接近度奖励
    """
